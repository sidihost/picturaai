...
function useSimulatedPlayback(duration: number, { loop = false, autoPlay = false } = {}) {
  ...
  const toggle = useCallback(() => {
    setIsPlaying((prev) => {
      if (!prev) setTime((t) => (t >= duration ? 0 : t))
      return !prev
    })
  }, [duration])

  const seekTo = useCallback((fraction: number) => {
    setTime(Math.min(Math.max(fraction, 0), 1) * duration)
  }, [duration])

  return { isPlaying, time, toggle, seekTo, setTime, setIsPlaying }
}

function useSeekBar(seekTo: (fraction: number) => void) {
  ...
  return { barRef, onSeek }
}

function VideoPreviewPlayer() {
  const { isPlaying, time, toggle, seekTo } = useSimulatedPlayback(VIDEO_DURATION, { loop: true })
  const { barRef, onSeek } = useSeekBar(seekTo)
  const progress = time / VIDEO_DURATION
  const frame = Math.min(Math.round(progress * 360), 360)

  return (
    <div
      className="relative aspect-video cursor-pointer bg-secondary/30 overflow-hidden"
      onClick={toggle}
      role="button"
      tabIndex={0}
    ...
    >
      <motion.div
        className="absolute inset-0"
        animate={isPlaying ? { scale: [1, 1.08], x: [0, -12] } : { scale: 1, x: 0 }}
        transition={isPlaying ? { duration: VIDEO_DURATION, ease: 'linear', repeat: Infinity } : { duration: 0.6 }}
      >
        <Image
      ...
      <div className="absolute left-3 top-3 flex items-center gap-1.5 rounded-full border border-white/15 bg-black/45 px-2.5 py-1 text-[10px] font-medium text-white backdrop-blur-md sm:left-4 sm:top-4">
        <span className="relative flex h-1.5 w-1.5">
          {isPlaying && <span className="absolute inset-0 animate-ping rounded-full bg-primary/70" />}
          <span className="relative inline-block h-1.5 w-1.5 rounded-full bg-primary" />
        </span>
        {isPlaying ? `Playing · frame ${frame} / 360` : 'Paused · tap to play'}
      </div>
      ...
      {/* Bottom controls bar */}
      <div
        className="absolute inset-x-3 bottom-3 rounded-xl border border-white/10 bg-black/50 px-3 py-2 backdrop-blur-md sm:inset-x-4 sm:bottom-4"
      ...
      >
        <div className="flex items-center gap-2.5">
          <button
            ...
            onClick={toggle}
            className="flex-shrink-0 text-white transition-opacity hover:opacity-80"
          >
          ...
          >
            ...
            <span
              className="absolute top-0 h-3 w-3 -translate-x-1/2 rounded-full bg-white shadow transition-transform group-hover:scale-110"
              style={{ left: `${progress * 100}%` }}
            />
          </div>
          <span className="font-mono text-[10px] text-white/80">{formatTime(time)} / {formatTime(VIDEO_DURATION)}</span>
...
const TRACK_DURATION = 168 // 2:48
const WAVEFORM = [0.32, 0.55, 0.78, 0.42, 0.68, 0.92, 0.6, 0.35, 0.82, 0.5, 0.72, 0.4, 0.58, 0.85, 0.5, 0.3, 0.65, 0.9, 0.45, 0.55, 0.75, 0.4, 0.6, 0.8, 0.45, 0.65, 0.88, 0.52, 0.36, 0.74, 0.48, 0.62, 0.86, 0.5, 0.3, 0.7, 0.42, 0.58, 0.82, 0.5]

function MusicPreviewPlayer() {
  const { isPlaying, time, toggle, seekTo } = useSimulatedPlayback(TRACK_DURATION)
  const { barRef, onSeek } = useSeekBar(seekTo)
      ...
      <div className="rounded-2xl border border-border/40 bg-card overflow-hidden shadow-lg">
        ...
        <div className="grid gap-0 sm:grid-cols-[180px_1fr]">
          ...
          <div className="relative aspect-square overflow-hidden bg-secondary/30 sm:aspect-auto">
            ...
            {/* Spinning vinyl accent — only spins while playing */}
            <motion.div
          ...
          <div className="flex flex-col justify-between gap-3 p-4 sm:p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                onClick={() => seekTo(Math.min((time + 15) / TRACK_DURATION, 1))}
                className="flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"
              >
                <SkipForward className="h-4 w-4 fill-current" />
              </button>
              <div className="ml-2 flex flex-1 items-center gap-2">
                <span className="font-mono text-[10px] text-muted-foreground">{formatTime(time)}</span>
                <div
                  ref={barRef}
                  onClick={onSeek}
                  role="slider"
                ...
                >
                  ...
                  <span
                    className="absolute top-0 h-3 w-3 -translate-x-1/2 rounded-full bg-primary shadow ring-2 ring-card transition-transform group-hover:scale-110"
                    style={{ left: `${progress * 100}%` }}
                  />
                </div>
                <span className="font-mono text-[10px] text-muted-foreground">{formatTime(TRACK_DURATION)}</span>
              </div>
        ...
        {/* Live animated waveform — animates while playing, seekable */}
        <div className="border-t border-border/30 bg-secondary/20 p-4 sm:p-5">
          ...
          >
            {WAVEFORM.map((h, i) => {
              const played = i / (WAVEFORM.length - 1) < progress
              return (
                <motion.span
                  key={i}
                  className={`flex-1 rounded-sm transition-colors duration-200 ${played ? 'bg-primary' : 'bg-primary/30'}`}
                  animate={isPlaying ? { height: [`${h * 100}%`, `${(1 - h * 0.55) * 100}%`, `${h * 100}%`] } : { height: `${h * 100}%` }}
                  transition={isPlaying ? { duration: 1.6 + (i % 5) * 0.2, repeat: Infinity, ease: 'easeInOut', delay: i * 0.04 } : { duration: 0.3 }}
                  style={{ height: `${h * 100}%` }}
                />
              )
            })}
